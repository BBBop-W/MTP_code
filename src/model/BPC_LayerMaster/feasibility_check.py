...
def dynamic_recursive_bs(
    ...
    """
    Backtracking Search (DFS) that mimics the logic of placing from center to outwards.
    It sorts cars by height descending, meaning the tallest cars MUST be placed in the center first.
    It then tries placing remaining cars in valid outer blocks, backtracking early if interval capacities are exceeded.
    """
    cars = []
...
if __name__ == "__main__":
    ...
    print("--- Feasibility Check Testing (Recursive BS vs Gurobi) ---")
    ...
    for case_id in range(tests):
        ...
        t0 = time.time()
        gurobi_res = check_layer_gurobi(compartment, deck_mode, quantities, car_lengths, car_heights, segments_data)
        t1 = time.time()
        bs_res = dynamic_recursive_bs(compartment, deck_mode, quantities, car_lengths, car_heights, segments_data)
        t2 = time.time()
        ...
        if match:
            ...
            print(f"Gurobi: {gurobi_res}, BS: {bs_res}")
            break
