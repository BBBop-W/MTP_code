...
def build_compartment_sequence(
...
) -> List[CompartmentRunItem]:
    """Build the 8 compartment runs in the required order.
    ...
    seq: List[CompartmentRunItem] = []
    for deck in ["h-h", "m-m", "m-h", "h-m"]:
        seq.append(
            CompartmentRunItem(
                deck=deck,
                compartment="upper",
                compartment_spec=_build_upper_compartment(deck, car_types, car_lengths, car_heights, max_quantity_by_type, max_units_per_type),
            )
        )
        seq.append(
            CompartmentRunItem(
                deck=deck,
                compartment="lower",
                compartment_spec=_build_lower_compartment(deck, car_types, car_lengths, car_heights, max_quantity_by_type, max_units_per_type),
            )
        )
    return seq
