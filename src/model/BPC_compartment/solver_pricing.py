...
from src.model.BPC_compartment import feasibility_check
from src.model.BPC_compartment.CG import MasterLPSolution, MasterProblem, PatternColumn
from src.model.BPC_compartment.labeling import DualValues, CompartmentSpec
        ...
        for p in modes:
            ...
            for compartment, duals, length_limit in [
                ...
                spec = CompartmentSpec(
                    compartment_id=f"{compartment}_{p}",
                    car_types=master.I,
                    car_lengths=master.length,
                    car_heights=car_heights,
                    compartment_length_limit=length_limit,
                ...
                for _ in range(self.max_columns_per_subproblem):
                    t0 = time.time()
                    result = solve_compartment_pricing_mip(
                        spec,
                        duals,
...
def solve_compartment_pricing_mip(
    ...
    compartment = str(compartment_spec.shape_params.get("compartment", "lower"))
    deck = str(compartment_spec.shape_params.get("deck", "h-h"))
    mode_left, mode_right = deck.split("-")

    segments = feasibility_check._get_segments_for_compartment(compartment_spec.car_heights)
    ...
    model = gp.Model("compartment_solver_pricing")
    model.Params.OutputFlag = 0
    Config.apply_gurobi_params(model)

    N = Config.max_units_per_compartment
    Delta = 400.0
    I = list(compartment_spec.car_types)
    ...
    for i in I:
        max_q = min(N, int(compartment_spec.max_quantity_by_type.get(i, N)))
    ...
    model.addConstr(gp.quicksum(x[i, h] for i in I for h in region_names) <= N, name="compartment_quantity")

    actual_limit_central = min(limit_left[0], limit_right[0])
    for i in I:
        if compartment_spec.car_heights[i] > actual_limit_central:
            model.addConstr(x[i, "central"] == 0, name=f"height[{i},central]")
        for idx in range(1, n_blocks + 1):
            if compartment_spec.car_heights[i] > limit_left[idx]:
                model.addConstr(x[i, f"left_{idx}"] == 0, name=f"height[{i},left_{idx}]")
            if compartment_spec.car_heights[i] > limit_right[idx]:
    ...
    for left_count in range(n_blocks + 1):
        for right_count in range(n_blocks + 1):
            ...
            model.addConstr(
                gp.quicksum(x[i, h] * (compartment_spec.car_lengths[i] + Delta) for i in I for h in interval_regions) <= cap
            )
    ...
    for i in I:
        coef = (
            compartment_spec.car_lengths[i]
