...
from src.model.BPC_LayerMaster.BBtree import normalize_car_table
    ...
    for stage, car_type in enumerate(car_types, start=1):
        ...
        for label in current:
            for q in range(0, max_q + 1):
                ...
                residuals = list(_placement_residuals(resource_model, label.residual, car_type, q, unit_resource))
...
def _print_row(row: Dict[str, object]) -> None:
    print(
        ...
        f"labels={row['labels_feasible']:9} dom={row['labels_pruned_by_dominance']:8} "
